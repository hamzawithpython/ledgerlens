...
- The lockfile must match the working environment: the proxies SDK-incompatibility bug, fixed locally during development, resurfaced in the Docker image because the fix lived in the local venv but requirements.txt still carried the old pin. The container builds only from requirements.txt, so "works on my machine" meant nothing until the pins were synced. Lesson: pin and commit dependency fixes immediately, and rebuild from the lockfile to catch drift.